...
## Card image layer fields

The live data and production data now support image-layer composition fields for rendering and export.

### Layer fields
- `card_layer_mode` — current value: `layered`
- `background_color_asset` — family background asset, or ultra override asset for Ultra cards
- `border_asset` — rarity border asset
- `front_background_image_asset` — shared front background image layer
- `front_main_asset` — shared front main/template asset
- `back_background_image_asset` — shared back background image layer
- `back_main_asset` — shared back main layer
- `front_template_asset` — front template card file
- `back_template_asset` — back template card file
- `image_asset_package` — current package: `card_images.zip`
- `ultra_visual_override` — boolean flag indicating ultra background/border override

### Engineering rule
These fields are rendering/export fields. They do not change gameplay rules, legality, or balance. Their purpose is to support layered card composition in build and production workflows.
