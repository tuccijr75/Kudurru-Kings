...
'@
Write-UTF8 $WirePath $WireCode

# ---------------- tools\kk\kk-restore.ps1 ----------------
$RestorePath = Join-Path $Tools "kk-restore.ps1"
$RestoreCode = @'
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
...
'@
Write-UTF8 $RestorePath $RestoreCode

Write-Host "Installing scripts to: $Tools" -ForegroundColor Cyan
Write-Host "Running kk-wire…" -ForegroundColor Cyan
& pwsh -NoLogo -NoProfile -File $WirePath

$srv = Join-Path $Root 'server'
$cli = Join-Path $Root 'client'
Write-Host ""
Write-Host "Next steps:" -ForegroundColor Yellow
Write-Host ("  SERVER: cd `"{0}`"; npm run dev" -f $srv) -ForegroundColor DarkGray
Write-Host ("  CLIENT: cd `"{0}`"; npm run dev" -f $cli) -ForegroundColor DarkGray
Write-Host "Open http://localhost:5173 and watch for [KK] logs in DevTools Console." -ForegroundColor DarkGray
# ========== END OF kk-bootstrap.ps1 ==========
